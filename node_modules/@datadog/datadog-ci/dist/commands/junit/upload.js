...
const library_1 = require("../git-metadata/library");
...
class UploadJUnitXMLCommand extends clipanion_1.Command {
    constructor() {
        ...
        this.reportMetrics = clipanion_1.Option.Array('--report-metrics', { hidden: true });
    ...
    getMatchingJUnitXMLFiles(spanTags, customTags, customMeasures, reportTags, reportMeasures) {
        return __awaiter(this, void 0, void 0, function* () {
            const jUnitXMLFiles = (this.basePaths || [])
                ...
                if (glob_1.default.hasMagic(basePath)) {
                    // It's a glob pattern so we just use it as is
                    globPattern = basePath;
                }
                else {
                    // It's a folder
                    globPattern = (0, utils_1.buildPath)(basePath, '*.xml');
                }
                const filesToUpload = glob_1.default.sync(globPattern).filter((file) => path_1.default.extname(file) === '.xml');
                return acc.concat(filesToUpload);
            }, [])
                .filter(utils_2.isFile);
            const validUniqueFiles = [...new Set(jUnitXMLFiles)].filter((jUnitXMLFilePath) => {
                ...
                if (validationErrorMessage) {
                    this.context.stdout.write((0, renderer_1.renderInvalidFile)(jUnitXMLFilePath, validationErrorMessage));
                    return false;
                }
                return true;
            });
    ...
    getCustomMeasures() {
        ...
        const cliMetrics = this.metrics ? (0, tags_1.parseMetrics)(this.metrics) : {};
        ...
        if (this.metrics) {
            this.context.stdout.write((0, renderer_1.renderDeprecatedMention)('--metrics', '--measures', 'option'));
        }
        if (this.config.envVarMetrics) {
            this.context.stdout.write((0, renderer_1.renderDeprecatedMention)('DD_METRICS', 'DD_MEASURES', 'environment variable'));
        }
        return Object.assign(Object.assign(Object.assign(Object.assign({}, cliMetrics), cliMeasures), envVarMetrics), envVarMeasures);
    }
    getReportTags() {
        return this.reportTags ? (0, tags_1.parseTags)(this.reportTags) : {};
    }
    getReportMeasures() {
        const cliMetrics = this.reportMetrics ? (0, tags_1.parseMetrics)(this.reportMetrics) : {};
        const cliMeasures = this.reportMeasures ? (0, tags_1.parseMetrics)(this.reportMeasures) : {};
        // We have renamed "metrics" to "measures", but we will still support the old names for now.
        if (this.reportMetrics) {
            this.context.stdout.write((0, renderer_1.renderDeprecatedMention)('--report-metrics', '--report-measures', 'option'));
        }
        return Object.assign(Object.assign({}, cliMetrics), cliMeasures);
    }
